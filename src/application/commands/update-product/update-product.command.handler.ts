...
import {
  OPTION_REPOSITORY,
  type IOptionRepository,
} from '~/domain/repositories/option.repository.interface'
import {
  OPTION_VALUE_REPOSITORY,
  type IOptionValueRepository,
} from '~/domain/repositories/option-value.repository.interface'
import {
...
export class UpdateProductHandler implements ICommandHandler<UpdateProductCommand, void> {
  ...
  async execute(command: UpdateProductCommand) {
    ...
    const { variantsToUpdate, newVariantEntities, variantsToCreate, variantsToDelete } =
  ...
  private async handleVariants(productId: string, body: any, shopId: string, tx: any) {
    ...
    for (const classification of body.classifications) {
      ...
      const existingOptionValues = await this.optionValueRepository.findByValues(
    ...
    if (body.classifications && body.classifications.length > 0) {
      ...
      if (optionsToDelete.length > 0) {
        ...
        const optionValuesToDelete =
          await this.optionValueRepository.findByOptionIds(optionIdsToDelete)
        if (optionValuesToDelete.length > 0) {
          ...
          await this.productVariantOptionValueRepository.deleteByOptionValueIds(
